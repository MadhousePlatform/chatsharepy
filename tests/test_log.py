...
class TestLogging(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.original_handlers = logger._core.handlers.copy()

    @classmethod
    def tearDownClass(cls):
        """Run once after all tests"""
        # Restore original logger configuration
        logger._core.handlers = cls.original_handlers
    ...
    def test_log_format(self):
        ...
        self.assertRegex(
            log_content,
            r'\d{2}:\d{2}:\d{2}\s+INFO\s+##\s+test_log\.py::test_log_format::\d+\s+##\s+' + test_message
        )
